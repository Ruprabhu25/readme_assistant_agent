...
describe("searchFiles", () => {
  ...
  it("returns no matches for a query that does not appear", async () => {
    const { workspace } = await fixture();
    const tool = makeSearchFilesTool(workspace);
    const result = await tool.execute!({ query: "zzz-nope-zzz", dir: "." }, toolCallOpts);
  ...
  it("restricts the search to the given subdirectory", async () => {
    ...
    await writeFile(path.join(root, "docs", "notes.md"), "add some notes here\n");

    const tool = makeSearchFilesTool(workspace);
    const result = await tool.execute!({ query: "add", dir: "src" }, toolCallOpts);
    expect(result.matches.every((m: { path: string }) => m.path.startsWith("src"))).toBe(true);
    expect(result.matches.some((m: { path: string }) => m.path.includes("docs"))).toBe(false);
  });

  it("never searches inside node_modules", async () => {
    const { workspace } = await fixture();
    const tool = makeSearchFilesTool(workspace);
    const result = await tool.execute!({ query: "module.exports", dir: "." }, toolCallOpts);
    expect(result.matches.some((m: { path: string }) => m.path.includes("node_modules"))).toBe(
      false,
    );
  });
  ...
  it("caps results at the maximum match count", async () => {
    const { workspace, root } = await fixture();
    const manyLines = Array.from({ length: 80 }, (_, i) => `needle line ${i}`).join("\n");
    await writeFile(path.join(root, "many.txt"), manyLines);

    const tool = makeSearchFilesTool(workspace);
    const result = await tool.execute!({ query: "needle", dir: "." }, toolCallOpts);
